	...
	while (1) {
		...
		if (cbow) {  //train the cbow architecture
			...
			for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
				...
				for (c = 0; c < layer1_size; c++) neu1[c] += syn0_action[c + last_action * layer1_size];
				cw++;
			}
			if (cw) {
				...
				if (hs) {
					...
					for (d = 0; d < vocab_entity[entity].codelen; d++) {
						f = 0;
						l2 = vocab_entity[entity].point[d] * layer1_size;
					...
					for (d = 0; d < vocab_action[action].codelen; d++) {
						f = 0;
						l2 = vocab_action[action].point[d] * layer1_size;
						// Propagate hidden -> output
				...
				for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
					...
					for (c = 0; c < layer1_size; c++) syn0_action[c + last_action * layer1_size] += neu1e[c];
				}
			}
			...
			for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
				...
				if (hs) {
					...
					for (d = 0; d < vocab_entity[entity].codelen; d++) {
						...
						for (c = 0; c < layer1_size; c++) f += syn0_action[c + l1_action] * syn1_entity[c + l2];
						if (f <= -MAX_EXP) continue;
						else if (f >= MAX_EXP) continue;
						else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
						// 'g' is the gradient multiplied by the learning rate
						...
						for (c = 0; c < layer1_size; c++) syn1_entity[c + l2] += g * syn0_action[c + l1_action];
					}
					for (d = 0; d < vocab_action[action].codelen; d++) {
						f = 0;
						l2 = vocab_action[action].point[d] * layer1_size;
						...
						for (c = 0; c < layer1_size; c++) f += syn0_action[c + l1_action] * syn1_action[c + l2];
						if (f <= -MAX_EXP) continue;
						else if (f >= MAX_EXP) continue;
						else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
						// 'g' is the gradient multiplied by the learning rate
						...
						for (c = 0; c < layer1_size; c++) syn1_action[c + l2] += g * syn0_action[c + l1_action];
					}
				}
				// NEGATIVE SAMPLING
				if (negative > 0) {
					...
					for (d = 0; d < negative + 1; d++) {
						...
						for (c = 0; c < layer1_size; c++) f += syn0_action[c + l1_action] * syn1neg_entity[c + l2];
						if (f > MAX_EXP) g = (label - 1) * alpha;
						else if (f < -MAX_EXP) g = (label - 0) * alpha;
						else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
						for (c = 0; c < layer1_size; c++) neu1e[c] += g * syn1neg_entity[c + l2];
						for (c = 0; c < layer1_size; c++) syn1neg_entity[c + l2] += g * syn0_entity[c + l1_entity];
						for (c = 0; c < layer1_size; c++) syn1neg_entity[c + l2] += g * syn0_action[c + l1_action];
					}
					for (d = 0; d < negative + 1; d++) {
						if (d == 0) {
							target = action;
						...
						for (c = 0; c < layer1_size; c++) f += syn0_action[c + l1_action] * syn1neg_action[c + l2];
						if (f > MAX_EXP) g = (label - 1) * alpha;
						else if (f < -MAX_EXP) g = (label - 0) * alpha;
						else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
						for (c = 0; c < layer1_size; c++) neu1e[c] += g * syn1neg_action[c + l2];
						for (c = 0; c < layer1_size; c++) syn1neg_action[c + l2] += g * syn0_entity[c + l1_entity];
						for (c = 0; c < layer1_size; c++) syn1neg_action[c + l2] += g * syn0_action[c + l1_action];
					}
				}
				// Learn weights input -> hidden
				for (c = 0; c < layer1_size; c++) syn0_entity[c + l1_entity] += neu1e[c];
				for (c = 0; c < layer1_size; c++) syn0_action[c + l1_action] += neu1e[c];
			}
		}
		sentence_position++;
