...
void InitNet() {
	...
	if (hs) {
		a = posix_memalign((void **)&syn1_entity, 128, (long long)vocab_size_entity * (layer1_size + layer1_size) * sizeof(real));
		if (syn1_entity == NULL) {printf("Memory allocation failed\n"); exit(1);}
		for (a = 0; a < vocab_size_entity; a++) for (b = 0; b < layer1_size + layer1_size; b++)
			syn1_entity[a * (layer1_size + layer1_size) + b] = 0;
	}
	if (negative>0) {
		a = posix_memalign((void **)&syn1neg_entity, 128, (long long)vocab_size_entity * (layer1_size + layer1_size) * sizeof(real));
		if (syn1neg_entity == NULL) {printf("Memory allocation failed\n"); exit(1);}
		for (a = 0; a < vocab_size_entity; a++) for (b = 0; b < layer1_size + layer1_size; b++)
			syn1neg_entity[a * (layer1_size + layer1_size) + b] = 0;
	}
	...
	if (hs) {
		a = posix_memalign((void **)&syn1_action, 128, (long long)vocab_size_action * (layer1_size + layer1_size) * sizeof(real));
		if (syn1_action == NULL) {printf("Memory allocation failed\n"); exit(1);}
		for (a = 0; a < vocab_size_action; a++) for (b = 0; b < layer1_size + layer1_size; b++)
			syn1_action[a * (layer1_size + layer1_size) + b] = 0;
	}
	if (negative>0) {
		a = posix_memalign((void **)&syn1neg_action, 128, (long long)vocab_size_action * (layer1_size + layer1_size)  * sizeof(real));
		if (syn1neg_action == NULL) {printf("Memory allocation failed\n"); exit(1);}
		for (a = 0; a < vocab_size_action; a++) for (b = 0; b < layer1_size + layer1_size; b++)
			syn1neg_action[a * (layer1_size + layer1_size) + b] = 0;
	}
...
void *TrainModelThread(void *id) {
	long long a, b, d, cw, entity, last_entity, action, last_action, sentence_length = 0, sentence_position = 0;
	long long behavior_count = 0, last_behavior_count = 0;
	long long l2, c, target, label, local_iter = iter;
	long long l1_entity, l1_action;
	unsigned long long next_random = (long long)id;
	real f, g;
	clock_t now;
	real *neu1 = (real *)calloc(layer1_size + layer1_size, sizeof(real));
	real *neu1e = (real *)calloc(layer1_size + layer1_size, sizeof(real));
	...
	while (1) {
		...
		if (sentence_length == 0 && fgets(buffer, MAX_RW_LINE_LENGTH, fi) != NULL) {
			...
			for (i=0; i<n; i+=2) {
				...
				sen_entity[sentence_length] = entity;
				sen_action[sentence_length] = action;
				sentence_length++;
			}
			behavior_count += sentence_length;
			sentence_position = 0;
			if (debug_mode > 2) {
				printf("new sentence with length=%lld behavior_count=%lld\n", sentence_length, behavior_count);
			}
		}
		if (feof(fi) || (behavior_count > train_behavior / num_threads)) {
			behavior_count_actual += behavior_count - last_behavior_count;
			local_iter--;
			if (debug_mode > 1)
		...
		entity = sen_entity[sentence_position];
		action = sen_action[sentence_position];
		if (debug_mode > 2) {
			printf("sentence_position=%d entity=%lld action=%lld\n", sentence_position, entity, action);
		}
		if (entity == -1 || action == -1) continue;
		for (c = 0; c < layer1_size + layer1_size; c++) neu1[c] = 0;
		for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] = 0;
		next_random = next_random * (unsigned long long)25214903917 + 11;
		...
		if (cbow) {  //train the cbow architecture
			...
			for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
				...
				last_entity = sen_entity[c];
				last_action = sen_action[c];
				if (last_entity == -1 || last_action == -1) continue;
				for (c = 0; c < layer1_size; c++) neu1[c] += syn0_entity[c + last_entity * layer1_size];
				for (c = 0; c < layer1_size; c++) neu1[layer1_size+c] += syn0_action[c + last_action * layer1_size];
				cw++;
			}
			if (cw) {
				for (c = 0; c < layer1_size + layer1_size; c++) neu1[c] /= cw;
				if (hs) {
					// For entity
					for (d = 0; d < vocab_entity[entity].codelen; d++) {
						f = 0;
						l2 = vocab_entity[entity].point[d] * (layer1_size + layer1_size);
						// Propagate hidden -> output
						for (c = 0; c < layer1_size + layer1_size; c++) f += neu1[c] * syn1_entity[c + l2];
						if (f <= -MAX_EXP) continue;
						else if (f >= MAX_EXP) continue;
						else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
						// 'g' is the gradient multiplied by the learning rate
						g = (1 - vocab_entity[entity].code[d] - f) * alpha;
						// Propagate errors output -> hidden
						for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] += g * syn1_entity[c + l2];
						// Learn weights hidden -> output
						for (c = 0; c < layer1_size + layer1_size; c++) syn1_entity[c + l2] += g * neu1[c];
					}
					// For action
					for (d = 0; d < vocab_action[action].codelen; d++) {
						f = 0;
						l2 = vocab_action[action].point[d] * (layer1_size + layer1_size);
						// Propagate hidden -> output
						for (c = 0; c < layer1_size + layer1_size; c++) f += neu1[c] * syn1_action[c + l2];
						if (f <= -MAX_EXP) continue;
						else if (f >= MAX_EXP) continue;
						else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
						// 'g' is the gradient multiplied by the learning rate
						g = (1 - vocab_action[action].code[d] - f) * alpha;
						// Propagate errors output -> hidden
						for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] += g * syn1_action[c + l2];
						// Learn weights hidden -> output
						for (c = 0; c < layer1_size + layer1_size; c++) syn1_action[c + l2] += g * neu1[c];
					}
				}
				// NEGATIVE SAMPLING
				if (negative > 0) {
					...
					for (d = 0; d < negative + 1; d++) {
						...
						l2 = target * (layer1_size + layer1_size);
						f = 0;
						for (c = 0; c < layer1_size + layer1_size; c++) f += neu1[c] * syn1neg_entity[c + l2];
						if (f > MAX_EXP) g = (label - 1) * alpha;
						else if (f < -MAX_EXP) g = (label - 0) * alpha;
						else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
						for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] += g * syn1neg_entity[c + l2];
						for (c = 0; c < layer1_size + layer1_size; c++) syn1neg_entity[c + l2] += g * neu1[c];
					}
					// For action
					for (d = 0; d < negative + 1; d++) {
						if (d == 0) {
						...
						l2 = target * (layer1_size + layer1_size);
						f = 0;
						for (c = 0; c < layer1_size + layer1_size; c++) f += neu1[c] * syn1neg_action[c + l2];
						if (f > MAX_EXP) g = (label - 1) * alpha;
						else if (f < -MAX_EXP) g = (label - 0) * alpha;
						else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
						for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] += g * syn1neg_action[c + l2];
						for (c = 0; c < layer1_size + layer1_size; c++) syn1neg_action[c + l2] += g * neu1[c];
					}
				}
				// hidden -> in
				for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
					...
					last_action = sen_action[c];
					if (last_entity == -1 || last_action == -1) continue;
					for (c = 0; c < layer1_size; c++) syn0_entity[c + last_entity * layer1_size] += neu1e[c];
					for (c = 0; c < layer1_size; c++) syn0_action[c + last_action * layer1_size] += neu1e[layer1_size + c];
				}
			}
			...
			for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
				...
				last_action = sen_action[c];
				if (last_entity == -1 || last_action == -1) continue;
				l1_entity = last_entity * layer1_size;
				l1_action = last_action * layer1_size;
				for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] = 0;
				...
				if (hs) {
					...
					for (d = 0; d < vocab_entity[entity].codelen; d++) {
						...
						l2 = vocab_entity[entity].point[d] * (layer1_size + layer1_size);
						// Propagate hidden -> output
						for (c = 0; c < layer1_size; c++) f += syn0_entity[c + l1_entity] * syn1_entity[c + l2];
						for (c = 0; c < layer1_size; c++) f += syn0_action[c + l1_action] * syn1_entity[c + layer1_size + l2];
						if (f <= -MAX_EXP) continue;
						else if (f >= MAX_EXP) continue;
						else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
						// 'g' is the gradient multiplied by the learning rate
						...
						for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] += g * syn1_entity[c + l2];
						// Learn weights hidden -> output
						for (c = 0; c < layer1_size; c++) syn1_entity[c + l2] += g * syn0_entity[c + l1_entity];
						for (c = 0; c < layer1_size; c++) syn1_entity[c + layer1_size + l2] += g * syn0_action[c + l1_action];
					}
					for (d = 0; d < vocab_action[action].codelen; d++) {
						f = 0;
						l2 = vocab_action[action].point[d] * (layer1_size + layer1_size);
						// Propagate hidden -> output
						for (c = 0; c < layer1_size; c++) f += syn0_entity[c + l1_entity] * syn1_action[c + l2];
						for (c = 0; c < layer1_size; c++) f += syn0_action[c + l1_action] * syn1_action[c + layer1_size + l2];
						if (f <= -MAX_EXP) continue;
						else if (f >= MAX_EXP) continue;
						else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
						// 'g' is the gradient multiplied by the learning rate
						...
						for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] += g * syn1_action[c + l2];
						// Learn weights hidden -> output
						for (c = 0; c < layer1_size; c++) syn1_action[c + l2] += g * syn0_entity[c + l1_entity];
						for (c = 0; c < layer1_size; c++) syn1_action[c + layer1_size + l2] += g * syn0_action[c + l1_action];
					}
				}
				// NEGATIVE SAMPLING
				if (negative > 0) {
					...
					for (d = 0; d < negative + 1; d++) {
						...
						l2 = target * (layer1_size + layer1_size);
						f = 0;
						for (c = 0; c < layer1_size; c++) f += syn0_entity[c + l1_entity] * syn1neg_entity[c + l2];
						for (c = 0; c < layer1_size; c++) f += syn0_action[c + l1_action] * syn1neg_entity[c + layer1_size + l2];
						if (f > MAX_EXP) g = (label - 1) * alpha;
						else if (f < -MAX_EXP) g = (label - 0) * alpha;
						else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
						for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] += g * syn1neg_entity[c + l2];
						for (c = 0; c < layer1_size; c++) syn1neg_entity[c + l2] += g * syn0_entity[c + l1_entity];
						for (c = 0; c < layer1_size; c++) syn1neg_entity[c + layer1_size + l2] += g * syn0_action[c + l1_action];
					}
					for (d = 0; d < negative + 1; d++) {
						if (d == 0) {
							target = action;
						...
						l2 = target * (layer1_size + layer1_size);
						f = 0;
						for (c = 0; c < layer1_size; c++) f += syn0_entity[c + l1_entity] * syn1neg_action[c + l2];
						for (c = 0; c < layer1_size; c++) f += syn0_action[c + l1_action] * syn1neg_action[c + layer1_size + l2];
						if (f > MAX_EXP) g = (label - 1) * alpha;
						else if (f < -MAX_EXP) g = (label - 0) * alpha;
						else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
						for (c = 0; c < layer1_size + layer1_size; c++) neu1e[c] += g * syn1neg_action[c + l2];
						for (c = 0; c < layer1_size; c++) syn1neg_action[c + l2] += g * syn0_entity[c + l1_entity];
						for (c = 0; c < layer1_size; c++) syn1neg_action[c + layer1_size + l2] += g * syn0_action[c + l1_action];
					}
				}
				// Learn weights input -> hidden
				for (c = 0; c < layer1_size; c++) syn0_entity[c + l1_entity] += neu1e[c];
				for (c = 0; c < layer1_size; c++) syn0_action[c + l1_action] += neu1e[c + layer1_size];
			}
		}
		sentence_position++;
